...
OUT="/var/lib/conector/xuper-adapter-v1-fix2.txt"
...
def as_bool(v):
    ...
    if isinstance(v, (int, float)):
        return int(bool(v))
    if isinstance(v, str):
        return int(v.strip().lower() in {"1","true","yes","y","si","sí"})
    return 0
...
def upsert(conn, item):
    ...
    canonical_id = "xuper:" + item["external_id"]
    conn.execute(
...
{
  echo "XUPER_ADAPTER_V1_FIX2_READY"
...
if command -v conector-publish-result >/dev/null 2>&1; then
  conector-publish-result "$OUT" "vm-results/xuper-adapter-v1-fix2.txt" || true
fi
