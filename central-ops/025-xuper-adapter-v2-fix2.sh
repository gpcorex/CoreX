...
OUT="/var/lib/conector/xuper-adapter-v2-fix2.txt"
...
NESTED_KEYS = {"simpleProgramList", "episodeList", "sameSeasonSeriesList", "subtitleList"}
...
def import_images(conn, vid, obj):
    sources = []
    if isinstance(obj.get("posterList"), list):
        sources.extend(obj["posterList"])
    for k in ("posterUrl","mIconPosterUrl"):
        if obj.get(k):
            sources.append(obj[k])
    ...
    for i, x in enumerate(sources):
        ...
        if isinstance(x, dict):
            ...
            if "back" in raw or "landscape" in raw:
                typ = "backdrop"
            elif "logo" in raw:
                typ = "logo"
            elif "thumb" in raw:
...
def import_tags(conn, vid, obj):
    ...
    for name in tag_values(obj):
        ...
        cur = conn.execute(
            "INSERT OR IGNORE INTO video_item_tag(video_item_id,tag_id) VALUES (?,?)",
            (vid, tid)
        )
        if cur.rowcount:
            count += 1
...
def import_episodes(conn, vid, obj):
    eps = []
    if isinstance(obj.get("simpleProgramList"), list):
        eps.extend(obj["simpleProgramList"])
    if isinstance(obj.get("episodeList"), list):
        eps.extend(obj["episodeList"])
...
TESTDB="/tmp/xuper-adapter-v2-fix2-test.db"
rm -f "$TESTDB"
...
ORPHANS="$(sqlite3 "$TESTDB" "SELECT count(*) FROM video_item WHERE id LIKE 'xuper:fixture-episode-%';")"
...
{
  echo "XUPER_ADAPTER_V2_FIX2_READY"
  ...
  echo "nested_episode_video_items=$ORPHANS"
...
if command -v conector-publish-result >/dev/null 2>&1; then
  conector-publish-result "$OUT" "vm-results/xuper-adapter-v2-fix2.txt" || true
fi
