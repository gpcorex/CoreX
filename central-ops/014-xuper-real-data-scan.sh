...
ROOTS="/home/ubuntu/Central /home/ubuntu/.cache /home/ubuntu/.local/share /srv/apps"
{
  ...
  grep -RIlE --include="*.json" --include="*.jsonl" --include="*.txt" --include="*.xml" --include="*.log" "contentId|programType|simpleProgramList|episodeList|posterList|channelCode" $ROOTS 2>/dev/null \
    | head -n 500
  echo
  echo "[SQLITE_DATABASES]"
  find $ROOTS -xdev -type f \( -iname "*.db" -o -iname "*.sqlite" -o -iname "*.sqlite3" \) -print 2>/dev/null | head -n 300
  echo
  echo "[SMALL_SAFE_JSON_SAMPLES]"
  while IFS= read -r f; do
    ...
    if [ "$sz" -le 1048576 ]; then
      if grep -qE "contentId|programType|simpleProgramList|episodeList|posterList|channelCode" "$f" 2>/dev/null; then
        echo "--- FILE: $f SIZE=$sz"
        sed -n "1,80p" "$f" | sed -E "s#(token|authorization|auth|license|password|secret|cookie)(["=: ]+)[^, }]+#\1\2<REDACTED>#Ig" | head -n 80
      fi
    fi
  done < <(grep -RIlE --include="*.json" --include="*.jsonl" --include="*.txt" "contentId|programType|simpleProgramList|episodeList|posterList|channelCode" $ROOTS 2>/dev/null | head -n 80)
} >"$OUT"
chmod 600 "$OUT"
