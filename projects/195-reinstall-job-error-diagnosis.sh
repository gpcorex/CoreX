...
BACKUP=/home/ubuntu/Central/backups/job-error-diagnosis-reinstall-$STAMP
mkdir -p "$BACKUP"
cp -a "$SERVER" "$BACKUP/server.py"

echo "=== 1. ENSURE DIAGNOSIS HELPER ==="
python3 - <<'PY'
...
if 'def diagnose_job_failure(job):' in s:
    s=re.sub(r'def diagnose_job_failure\(job\):.*?\nclass H\(BaseHTTPRequestHandler\):',
             helper+'class H(BaseHTTPRequestHandler):',s,flags=re.S)
else:
...
echo "=== 2. ENSURE JOB DIAGNOSIS ENDPOINT ==="
python3 - <<'PY'
from pathlib import Path
p=Path("/home/ubuntu/Interfaz/server.py")
s=p.read_text(encoding="utf-8")
...
'''
    insert='''        if p=="/api/job-diagnose":
        ...
        if p=="/api/message":
            try:
'''
    if anchor not in s: raise SystemExit("POST_MESSAGE_ANCHOR_NOT_FOUND")
    s=s.replace(anchor,insert,1)

p.write_text(s,encoding="utf-8")
PY

echo "=== 3. ENSURE ERROR UI ==="
python3 - <<'PY'
from pathlib import Path
p=Path("/home/ubuntu/Interfaz/server.py")
s=p.read_text(encoding="utf-8")
...
if '.jobErrorDetail{' not in s:
    ...
    new=anchor+'''.jobErrorDetail{margin-top:10px;padding:10px;border-radius:10px;background:#23191b;border:1px solid #573238;white-space:pre-wrap;font-size:13px}.jobDiagnosis{margin-top:10px;padding:10px;border-radius:10px;background:var(--panel2);border:1px solid var(--line);white-space:pre-wrap;font-size:13px;line-height:1.45}.jobDiagTitle{font-weight:700;margin-bottom:6px}.jobDiagWait{color:var(--muted);font-size:12px;margin-top:8px}'''
    if anchor not in s: raise SystemExit("JOB_CSS_ANCHOR_NOT_FOUND")
...
echo JOB_DIAGNOSIS_FULL_PATCH_OK

echo "=== 4. VERIFY ALL COMPONENTS ==="
for marker in   'def diagnose_job_failure'   'JOB_NOT_IN_ERROR'   'jobErrorDetail'   'Diagnóstico y solución sugerida'   'api/job-diagnose'
do
...
for i in $(seq 1 30); do
  if curl -fsS --max-time 2 http://127.0.0.1:8791/api/health >/tmp/jobdiag-reinstall-health.json 2>/dev/null; then break; fi
  sleep 1
done
cat /tmp/jobdiag-reinstall-health.json
echo
systemctl is-active interfaz.service
echo INTERFAZ_JOB_DIAGNOSIS_SERVICE_OK

echo "=== 6. PUBLIC UI MARKERS ==="
