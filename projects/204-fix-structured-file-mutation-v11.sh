...
BACKUP=/home/ubuntu/Central/backups/classifier-structured-guard-v11-$STAMP
mkdir -p "$BACKUP"
cp -a "$CLI" "$BACKUP/cli.py"

echo "=== 1. GUARD STRUCTURED OUTPUT AGAINST EXPLICIT FILE MUTATION ==="
python3 - <<'PY'
...
'''
new='''    explicit_file_mutation = (
        any(v in t for v in file_action_words)
        and re.search(r'\\b[\\w.-]+\\.(?:txt|md|json|csv|py|js|ts|sh|log|yaml|yml|xml|html|css|ini|conf)\\b', t)
    )
    if any(w in t for w in structured_words) and not explicit_file_mutation:
        return "estructurado"
'''

if old not in s:
    raise SystemExit("STRUCTURED_DECISION_LINE_NOT_FOUND")

p.write_text(s.replace(old,new,1),encoding="utf-8")
PY

python3 -m py_compile "$CLI"
echo CLASSIFIER_STRUCTURED_GUARD_PATCH_OK
...
print("CLASSIFIER_STRUCTURED_GUARD_V11_TESTS_OK")
PY

echo "=== 3. LIVE JSON FILE JOB ==="
RESP=$(curl -fsS --max-time 10 -H 'Content-Type: application/json'   -d '{"task":"Generá classifier-v11.json con el contenido exacto {\"ok\":true}, leelo y verificá que coincida exactamente.","source":"chat","project":"Central","conversation_id":"classifier-v11"}'   http://127.0.0.1:8091/api/jobs)
echo "$RESP"
...
test -f "/home/ubuntu/Central/work/$JOB/classifier-v11.json"
python3 - "$JOB" <<'PY'
import json,sys
p=f"/home/ubuntu/Central/work/{sys.argv[1]}/classifier-v11.json"
x=json.load(open(p,encoding="utf-8"))
assert x=={"ok":True},x
print("FILE_CREATE_JSON_CONTENT_OK")
PY

echo CENTRAL_CLASSIFIER_STRUCTURED_GUARD_V11_READY
echo "backup=$BACKUP"
