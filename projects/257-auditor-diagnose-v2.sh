...

{
  ...
  grep -RIl --exclude-dir='.git' -E 'Últimos|ultimos|stage_total|progress|log' /home/ubuntu/Central 2>/dev/null | grep -E '\.(py|js|ts|tsx|jsx|html|css)$' | head -100 || true
  echo
  echo "[RUN_PIPELINE_EXCERPT]"
  PIPE=$(find /home/ubuntu/Central -type f -name 'run_central_extraction.py' -print -quit 2>/dev/null || true)
  if [ -n "$PIPE" ]; then nl -ba "$PIPE" | sed -n '1,280p'; fi
  echo
  echo "[NO_INTERFACE_EXCERPTS]"
  while IFS= read -r f; do
    [ -n "$f" ] || continue
    echo "--- FILE: $f ---"
    grep -n -C 25 'NO_INTERFACE_OPERATIONS' "$f" || true
